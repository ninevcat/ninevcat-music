		...
		.end(function(err, _response) {
			...
			if (!err) {
				...
				$('.m-cvrlst').eq(0).find('li').each(function(index, element) {
					...
					// 获得 cover 歌单封面
					var cover = $(element).find('.u-cover').find('img').attr('src').slice(0, -7).concat("162y162");;
					...
					var recommendItem = {
						...
						songhref: 'http://localhost:4000/song_list/' + cvrLink.attr('data-res-id'),
						listhref: 'http://localhost:4000/playlist/' + cvrLink.attr('data-res-id'),
						songidhref: 'http://localhost:4000/song/' + cvrLink.attr('data-res-id'),
	...
	request.get(`http://music.163.com/playlist?id=${playlistId}`)
		.end(function(err, _response) {
			...
			if (!err) {
				...
				// playlist.songid = dom.find('#song-list-pre-cache').find('.f-hide').html()
				// 播放总数量
...
// 定义根据歌单id获得歌单所有歌曲mp3地址的API
app.get('/song/:playlistId', function(req, res) {
	...
	request.get(`http://music.163.com/playlist?id=${playlistId}`)
		.end(function(err, _response) {
			...
			if (!err) {
				...
				var songidList = [];
				// 获得 .f-hide 的 ul 元素
				$('#m-playlist').find('#song-list-pre-cache').find('.f-hide').find('li').each(function(index, element) {
					// 获得 a 链接
					var cvrLink = $(element).find('a');
					// 组织单个推荐歌单对象结构
					var recommendItem = {
						id: cvrLink.attr('href').slice(9),
						href: 'http://localhost:4000/songmp3/' + cvrLink.attr('href').slice(9),
						url: 'http://music.163.com' + cvrLink.attr('href')
					};
					// 将单个对象放在数组中
					songidList.push(recommendItem);

				});



				resObj.data = songidList;
